                  ...
                  AdafruitIO_WiFi* io)
{
  ...
  tft -> fillRect(5, 220, 150, 20, TFT_BACKGROUND);
  tft -> setCursor(5, 220);
...
void postsCounter(  TFT_eSPI* tft)
{
  ...
  tft->loadFont("NotoSansBold15");
  tft->setTextColor(TFT_LIGHTGREY, TFT_BACKGROUND);
  tft->fillRect(220, 0, 90, 20, TFT_BACKGROUND);
  tft->setCursor(220, 0);
...
void readTouch(TFT_eSPI* tft)
{
  ...
  if (tft->getTouch(&x, &y)) {
    if (x < 3000 && y < 16000)
    {
      tft->fillRect(220, 160, 70, 40, TFT_BACKGROUND);
...
void heapSize(TFT_eSPI* tft)
{
  ...
  tft->fillRect(220, 140, 55, 20, TFT_BACKGROUND);
